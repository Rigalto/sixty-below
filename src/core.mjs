...
import {NODES, NODES_LOOKUP, NODE_TYPE, SKY_BORDER_NODE, MAX_FURNITURE_W, MAX_FURNITURE_H, ITEM_TYPE, ITEMS, BUFFS, RECIPES, MONSTERS, PLANT_KIND, PLANT_TYPE, PLANT_SYSTEM_LOOKUP, ALL_PLANT_SYSTEMS, TREE_IMAGES, BLOCKED_ICON} from '../assets/data/data.mjs'
...
const plantSystemLookup = [ // Map<kind*100+type, system> — peuplée au fur et à mesure
  ...
  [PLANT_KIND.HERB * 100 + PLANT_TYPE.SATANS_CUBE, satansCubeSystem],
  [PLANT_KIND.HERB * 100 + PLANT_TYPE.SNEAKTHORN, sneakthornSystem],
  [PLANT_KIND.HERB * 100 + PLANT_TYPE.CURSEDCROWN, cursedcrownSystem],
  [PLANT_KIND.HERB * 100 + PLANT_TYPE.ABYSSHORN, abysshornSystem],
...
class GameCore {
  ...
  #hydrateItems () {
    ...
    BLOCKED_ICON.image = resolveAssetData(BLOCKED_ICON.image)
 ...
 */
  #hydrateHelp () {
    const {count, errors} = hydrateHelp(NODES, ITEMS, BUFFS, RECIPES)
 ...
 */
  ...
  #runDebugAction () {
    ...
    bambooSystem.debug()
    pricklepadSystem.debug()
    satansCubeSystem.debug()
    sneakthornSystem.debug()
    cursedcrownSystem.debug()
    abysshornSystem.debug()
    inferncapSystem.debug()
