...
/* ====================================================================================================
   ...
   Singleton : sneakthornSystem.

   Population constante : #list reçoit en une fois (init) le tableau complet des sneakthorns,
   taille fixe jamais réallouée (pas de GC). Un record present=false signale un slot à faire
   repousser ailleurs — mis en #regrowQueue, vidée par microtâche (sneakthornRegrow).

   ==================================================================================================== */

const SNEAKTHORN_REGROW_INITIAL_DELAY_MS = 1549
const SNEAKTHORN_REGROW_RETRY_DELAY_MS = 131

class SneakthornSystem {
  byTile = new Map() // Map<tileIndex, record> — public : membership O(1) + lookup record
  #list = [] // record[] — population fixe, référence affectée dans init(records)
  #byChunk = new Map() // Map<chunkKey, Set> — lookup spatial pour onPreloadChunksChanged
  #bySoil = new Map() // Map<soilIndex, record> — sneakthorns présents : détection retrait du sol (2 entrées/record, sol sur 2 tuiles)
  #displayed = new Set() // Set<record> — cible du render (chunks preload uniquement)
  #regrowQueue = [] // record[] — records present=false en attente d'un nouvel emplacement
  #image = null // image à afficher (mise en cache)
  ...
  constructor () {
    // eventBus
    this.onFirstLoopSneakthorn = this.onFirstLoopSneakthorn.bind(this)
    eventBus.on('time/first-loop', this.onFirstLoopSneakthorn)
    this.onTileChangedSneakthorn = this.onTileChangedSneakthorn.bind(this)
    eventBus.on('world/tile-changed', this.onTileChangedSneakthorn)
    // micro-tâches
    this.sneakthornRegrow = this.sneakthornRegrow.bind(this)
  }
  ...
  init () {
    ...
    this.#image = ITEMS.sneakthorn.placed // après hydratation
  }

  /**
   * Enregistre un sneakthorn et peuple les structures internes.
   * Si present=false, met le record en file de repousse.
   * @param {object} record — record HERB/SNEAKTHORN (deleted=false garanti par l'appelant)
   */
  initPlant (record) {
    this.#list.push(record)
  ...
  debug () {
    console.log(`[SneakthornSystem] ${this.#list.length} sneakthorns récupérés, ${this.#regrowQueue.length} en attente de repousse`)
  }
   ...
   * Délai long pour ne pas surcharger le démarrage.
   */
  onFirstLoopSneakthorn () {
    if (this.#regrowQueue.length === 0) return
    const {priority, capacity} = MICROTASK.SNEAKTHORN_REGROW
    taskScheduler.enqueue('sneakthorn-regrow', SNEAKTHORN_REGROW_INITIAL_DELAY_MS, this.sneakthornRegrow, priority, capacity)
  }
  ...
  /**
   * Dessine les sneakthorns visibles et présents sur le contexte transformé.
   * @param {CanvasRenderingContext2D} ctx — contexte déjà transformé (caméra appliquée)
   */
  render (ctx) {
    const img = this.#image
  ...
  /**
   * Retourne le record de sneakthorn couvrant la tuile donnée, ou null.
   * @param {number} tileIndex — (y << 10) | x
   * @returns {object|null}
   */
  getPlantAt (tileIndex) {
  ...
  /**
   * Détruit un sneakthorn présent sans loot : retire byTile/#byChunk/#bySoil (2 entrées)/
   * #displayed, débloque le rectangle 2x2 occupé, persiste, puis programme la repousse
   * (#regrowQueue + microtâche). Guard : no-op si record.present est déjà false.
   * @param {object} record
   */
  #destroyPresent (record) {
    ...
    if (this.#regrowQueue.length === 1) {
      const {priority, capacity} = MICROTASK.SNEAKTHORN_REGROW
      taskScheduler.enqueue('sneakthorn-regrow', SNEAKTHORN_REGROW_RETRY_DELAY_MS, this.sneakthornRegrow, priority, capacity)
    }
  }

  /**
   * Traite le foraging réussi de ce sneakthorn (hors loot, géré par ForagingManager).
   * Marque le record absent et programme la repousse.
   * @param {object} record
   */
  onForaged (record) {
   ...
   * (TOPSOIL_Y_UNDER_CAVERNS → TOPSOIL_Y_CAVERNS_MID), descend jusqu'à la première tuile
   * non-VOID.
   * @returns {number} index packé (y<<10)|x du sol si HARDSTONE ou SLATE trouvé, 0 sinon
   */
  #findSneakthornFloor () {
    const VOID = NODES.VOID.code
    const HARDSTONE = NODES.HARDSTONE.code
    ...
    const support = chunkManager.getTileAt(idx)
    if (support !== HARDSTONE && support !== SLATE) return 0

    return idx
  }

  /**
   * Teste les deux côtés du sol trouvé (HARDSTONE ou SLATE adjacent + pocket 2x2 VOID
   * au-dessus + tuiles libres de tout blocage).
   * @param {number} floorIndex — retour de #findSneakthornFloor
   * @returns {number} soilX retenu, 0 si aucun côté valide
   */
  #findSneakthornSide (floorIndex) {
    const VOID = NODES.VOID.code
    const HARDSTONE = NODES.HARDSTONE.code
    const SLATE = NODES.SLATE.code
    const cx = floorIndex & 0x3FF
    ...
    const supportRight = chunkManager.getTileAt(floorIndex + 1)
    const canRight = (supportRight === HARDSTONE || supportRight === SLATE) &&
      chunkManager.isRectCode(cx, topY, 2, 2, VOID) &&
      blockedTiles.canPlaceRect(cx, topY, 2, 2)

    const supportLeft = chunkManager.getTileAt(floorIndex - 1)
    const canLeft = (supportLeft === HARDSTONE || supportLeft === SLATE) &&
      chunkManager.isRectCode(cx - 1, topY, 2, 2, VOID) &&
      blockedTiles.canPlaceRect(cx - 1, topY, 2, 2)
  ...
  /**
   * Cherche un nouvel emplacement pour le dernier record de #regrowQueue.
   * Départ VOID, descente jusqu'au premier non-VOID, sol HARDSTONE ou SLATE (ancre, via
   * #findSneakthornFloor), puis test flat à droite (cx+1) et à gauche (cx-1) via
   * #findSneakthornSide — 50/50 si les deux sont valides.
   * Si trouvé : finalise le record (present=true, byTile/#byChunk/#bySoil, blockPlacement,
   * persistence) et le retire de #regrowQueue (dernier élément, length--).
   * Replanifie tant que #regrowQueue n'est pas vide, avec SNEAKTHORN_REGROW_RETRY_DELAY_MS.
   */
  sneakthornRegrow () {
    if (this.#regrowQueue.length === 0) return

    const floorIndex = this.#findSneakthornFloor()
    const soilX = floorIndex !== 0 ? this.#findSneakthornSide(floorIndex) : 0
    const found = soilX !== 0
  ...
  /**
   * Liaison EventBus : 'world/tile-changed'.
   * Détruit le sneakthorn présent si une des 4 tuiles VOID de son corps n'est plus libre,
   * ou si l'une des 2 tuiles sol n'est plus HARDSTONE ni SLATE.
   * @param {{tileIndex: number, tileOldCode: number, tileNewCode: number}} payload
   */
  onTileChangedSneakthorn ({tileIndex}) {
    // Cas 1 — tuile du corps : une des 4 VOID devient autre chose
    const byBodyRecord = this.byTile.get(tileIndex)
    if (byBodyRecord !== undefined) this.#destroyPresent(byBodyRecord)
...
/* ====================================================================================================
   ...
   Singleton : abysshornSystem.

   Population constante : #list reçoit en une fois (init) le tableau complet des abysshorns,
   taille fixe jamais réallouée (pas de GC). Un record present=false signale un slot à faire
   repousser ailleurs — mis en #regrowQueue, vidée par microtâche (abysshornRegrow).

   ==================================================================================================== */

const ABYSSHORN_REGROW_INITIAL_DELAY_MS = 1559
const ABYSSHORN_REGROW_RETRY_DELAY_MS = 133

class AbysshornSystem {
  byTile = new Map() // Map<tileIndex, record> — public : membership O(1) + lookup record
  #list = [] // record[] — population fixe, référence affectée dans init(records)
  #byChunk = new Map() // Map<chunkKey, Set> — lookup spatial pour onPreloadChunksChanged
  #bySoil = new Map() // Map<soilIndex, record> — abysshorns présents : détection retrait du sol (2 entrées/record, sol sur 2 tuiles)
  #displayed = new Set() // Set<record> — cible du render (chunks preload uniquement)
  #regrowQueue = [] // record[] — records present=false en attente d'un nouvel emplacement
  #image = null // image à afficher (mise en cache)
  ...
  constructor () {
    // eventBus
    this.onFirstLoopAbysshorn = this.onFirstLoopAbysshorn.bind(this)
    eventBus.on('time/first-loop', this.onFirstLoopAbysshorn)
    this.onTileChangedAbysshorn = this.onTileChangedAbysshorn.bind(this)
    eventBus.on('world/tile-changed', this.onTileChangedAbysshorn)
    // micro-tâches
    this.abysshornRegrow = this.abysshornRegrow.bind(this)
  }
  ...
  init () {
    ...
    this.#image = ITEMS.abysshorn.placed // après hydratation
  }

  /**
   * Enregistre un abysshorn et peuple les structures internes.
   * Si present=false, met le record en file de repousse.
   * @param {object} record — record HERB/ABYSSHORN (deleted=false garanti par l'appelant)
   */
  initPlant (record) {
    this.#list.push(record)
  ...
  debug () {
    console.log(`[AbysshornSystem] ${this.#list.length} abysshorns récupérés, ${this.#regrowQueue.length} en attente de repousse`)
  }
   ...
   * Délai long pour ne pas surcharger le démarrage.
   */
  onFirstLoopAbysshorn () {
    if (this.#regrowQueue.length === 0) return
    const {priority, capacity} = MICROTASK.ABYSSHORN_REGROW
    taskScheduler.enqueue('abysshorn-regrow', ABYSSHORN_REGROW_INITIAL_DELAY_MS, this.abysshornRegrow, priority, capacity)
  }
  ...
  /**
   * Dessine les abysshorns visibles et présents sur le contexte transformé.
   * @param {CanvasRenderingContext2D} ctx — contexte déjà transformé (caméra appliquée)
   */
  render (ctx) {
    const img = this.#image
  ...
  /**
   * Retourne le record d'abysshorn couvrant la tuile donnée, ou null.
   * @param {number} tileIndex — (y << 10) | x
   * @returns {object|null}
   */
  getPlantAt (tileIndex) {
  ...
  /**
   * Détruit un abysshorn présent sans loot : retire byTile/#byChunk/#bySoil (2 entrées)/
   * #displayed, débloque le rectangle 2x2 occupé, persiste, puis programme la repousse
   * (#regrowQueue + microtâche). Guard : no-op si record.present est déjà false.
   * @param {object} record
   */
  #destroyPresent (record) {
    ...
    if (this.#regrowQueue.length === 1) {
      const {priority, capacity} = MICROTASK.ABYSSHORN_REGROW
      taskScheduler.enqueue('abysshorn-regrow', ABYSSHORN_REGROW_RETRY_DELAY_MS, this.abysshornRegrow, priority, capacity)
    }
  }

  /**
   * Traite le foraging réussi de cet abysshorn (hors loot, géré par ForagingManager).
   * Marque le record absent et programme la repousse.
   * @param {object} record
   */
  onForaged (record) {
   ...
   * (TOPSOIL_Y_UNDER_CAVERNS → TOPSOIL_Y_CAVERNS_MID), descend jusqu'à la première tuile
   * non-VOID.
   * @returns {number} index packé (y<<10)|x du sol si HARDSTONE, HELLSTONE ou SLATE trouvé, 0 sinon
   */
  #findAbysshornFloor () {
    const VOID = NODES.VOID.code
    const HARDSTONE = NODES.HARDSTONE.code
    ...
    const support = chunkManager.getTileAt(idx)
    if (support !== HARDSTONE && support !== HELLSTONE && support !== SLATE) return 0

    return idx
  }

  /**
   * Teste les deux côtés du sol trouvé (HARDSTONE, HELLSTONE ou SLATE adjacent + pocket
   * 2x2 VOID au-dessus + tuiles libres de tout blocage).
   * @param {number} floorIndex — retour de #findAbysshornFloor
   * @returns {number} soilX retenu, 0 si aucun côté valide
   */
  #findAbysshornSide (floorIndex) {
    const VOID = NODES.VOID.code
    const HARDSTONE = NODES.HARDSTONE.code
    const HELLSTONE = NODES.HELLSTONE.code
    const SLATE = NODES.SLATE.code
    ...
    const supportRight = chunkManager.getTileAt(floorIndex + 1)
    const canRight = (supportRight === HARDSTONE || supportRight === HELLSTONE || supportRight === SLATE) &&
      chunkManager.isRectCode(cx, topY, 2, 2, VOID) &&
      blockedTiles.canPlaceRect(cx, topY, 2, 2)

    const supportLeft = chunkManager.getTileAt(floorIndex - 1)
    const canLeft = (supportLeft === HARDSTONE || supportLeft === HELLSTONE || supportLeft === SLATE) &&
      chunkManager.isRectCode(cx - 1, topY, 2, 2, VOID) &&
      blockedTiles.canPlaceRect(cx - 1, topY, 2, 2)
  ...
  /**
   * Cherche un nouvel emplacement pour le dernier record de #regrowQueue.
   * Départ VOID, descente jusqu'au premier non-VOID, sol HARDSTONE, HELLSTONE ou SLATE
   * (ancre, via #findAbysshornFloor), puis test flat à droite (cx+1) et à gauche (cx-1)
   * via #findAbysshornSide — 50/50 si les deux sont valides.
   * Si trouvé : finalise le record (present=true, byTile/#byChunk/#bySoil, blockPlacement,
   * persistence) et le retire de #regrowQueue (dernier élément, length--).
   * Replanifie tant que #regrowQueue n'est pas vide, avec ABYSSHORN_REGROW_RETRY_DELAY_MS.
   */
  abysshornRegrow () {
    if (this.#regrowQueue.length === 0) return

    const floorIndex = this.#findAbysshornFloor()
    const soilX = floorIndex !== 0 ? this.#findAbysshornSide(floorIndex) : 0
    const found = soilX !== 0
  ...
  /**
   * Liaison EventBus : 'world/tile-changed'.
   * Détruit l'abysshorn présent si une des 4 tuiles VOID de son corps n'est plus libre,
   * ou si l'une des 2 tuiles sol n'est plus HARDSTONE, HELLSTONE ni SLATE.
   * @param {{tileIndex: number, tileOldCode: number, tileNewCode: number}} payload
   */
  onTileChangedAbysshorn ({tileIndex}) {
    // Cas 1 — tuile du corps : une des 4 VOID devient autre chose
    const byBodyRecord = this.byTile.get(tileIndex)
    if (byBodyRecord !== undefined) this.#destroyPresent(byBodyRecord)
...
export const abysshornSystem = new AbysshornSystem()

/* ====================================================================================================
   INFERNCAP SYSTEM
   ====================================================================================================

   Singleton : inferncapSystem.

   Population constante : #list reçoit en une fois (init) le tableau complet des inferncaps,
   taille fixe jamais réallouée (pas de GC). Un record present=false signale un slot à faire
   repousser ailleurs — mis en #regrowQueue, vidée par microtâche (inferncapRegrow).

   ==================================================================================================== */

const INFERNCAP_REGROW_INITIAL_DELAY_MS = 1567
const INFERNCAP_REGROW_RETRY_DELAY_MS = 139

class InferncapSystem {
  byTile = new Map() // Map<tileIndex, record> — public : membership O(1) + lookup record
  #list = [] // record[] — population fixe, référence affectée dans init(records)
  #byChunk = new Map() // Map<chunkKey, Set> — lookup spatial pour onPreloadChunksChanged
  #bySoil = new Map() // Map<soilIndex, record> — inferncaps présents : détection retrait du sol (2 entrées/record, sol sur 2 tuiles)
  #displayed = new Set() // Set<record> — cible du render (chunks preload uniquement)
  #regrowQueue = [] // record[] — records present=false en attente d'un nouvel emplacement
  #image = null // image à afficher (mise en cache)
  ...
  constructor () {
    // eventBus
    this.onFirstLoopInferncap = this.onFirstLoopInferncap.bind(this)
    eventBus.on('time/first-loop', this.onFirstLoopInferncap)
    this.onTileChangedInferncap = this.onTileChangedInferncap.bind(this)
    eventBus.on('world/tile-changed', this.onTileChangedInferncap)
    // micro-tâches
    this.inferncapRegrow = this.inferncapRegrow.bind(this)
  }
  ...
  init () {
    ...
    this.#image = ITEMS.inferncap.placed // après hydratation
  }

  /**
   * Enregistre un inferncap et peuple les structures internes.
   * Si present=false, met le record en file de repousse.
   * @param {object} record — record HERB/INFERNCAP (deleted=false garanti par l'appelant)
   */
  initPlant (record) {
    this.#list.push(record)
  ...
  debug () {
    console.log(`[InferncapSystem] ${this.#list.length} inferncaps récupérés, ${this.#regrowQueue.length} en attente de repousse`)
  }
   ...
   * Délai long pour ne pas surcharger le démarrage.
   */
  onFirstLoopInferncap () {
    if (this.#regrowQueue.length === 0) return
    const {priority, capacity} = MICROTASK.INFERNCAP_REGROW
    taskScheduler.enqueue('inferncap-regrow', INFERNCAP_REGROW_INITIAL_DELAY_MS, this.inferncapRegrow, priority, capacity)
  }
  ...
  /**
   * Dessine les inferncaps visibles et présents sur le contexte transformé.
   * @param {CanvasRenderingContext2D} ctx — contexte déjà transformé (caméra appliquée)
   */
  render (ctx) {
    const img = this.#image
  ...
  /**
   * Retourne le record d'inferncap couvrant la tuile donnée, ou null.
   * @param {number} tileIndex — (y << 10) | x
   * @returns {object|null}
   */
  getPlantAt (tileIndex) {
  ...
  /**
   * Détruit un inferncap présent sans loot : retire byTile/#byChunk/#bySoil (2 entrées)/
   * #displayed, débloque le rectangle 2x3 occupé, persiste, puis programme la repousse
   * (#regrowQueue + microtâche). Guard : no-op si record.present est déjà false.
   * @param {object} record
   */
  #destroyPresent (record) {
    ...
    if (this.#regrowQueue.length === 1) {
      const {priority, capacity} = MICROTASK.INFERNCAP_REGROW
      taskScheduler.enqueue('inferncap-regrow', INFERNCAP_REGROW_RETRY_DELAY_MS, this.inferncapRegrow, priority, capacity)
    }
  }

  /**
   * Traite le foraging réussi de cet inferncap (hors loot, géré par ForagingManager).
   * Marque le record absent et programme la repousse.
   * @param {object} record
   */
  onForaged (record) {
  ...
  /**
   * Tire une colonne et une hauteur au hasard dans la bande Caverns Bottom
   * (TOPSOIL_Y_CAVERNS_MID → WORLD_HEIGHT - 1), descend jusqu'à la première tuile non-VOID.
   * @returns {number} index packé (y<<10)|x du sol si HARDSTONE, HELLSTONE ou SLATE trouvé, 0 sinon
   */
  #findInferncapFloor () {
    const VOID = NODES.VOID.code
    ...
    const SLATE = NODES.SLATE.code
    const W = WORLD_WIDTH
    const yBottom = WORLD_HEIGHT - 1

    const cx = seededRNG.randomGetMinMax(2, W - 3)
    const cy = seededRNG.randomGetMinMax(TOPSOIL_Y_CAVERNS_MID, yBottom)
    let idx = (cy << 10) | cx

    if (chunkManager.getTileAt(idx) !== VOID) return 0
    const maxIndex = (yBottom << 10) | cx
    while (idx < maxIndex && chunkManager.getTileAt(idx) === VOID) idx += W
  ...
  /**
   * Teste les deux côtés du sol trouvé (HARDSTONE, HELLSTONE ou SLATE adjacent + pocket
   * 2x3 VOID au-dessus + tuiles libres de tout blocage).
   * @param {number} floorIndex — retour de #findInferncapFloor
   * @returns {number} soilX retenu, 0 si aucun côté valide
   */
  #findInferncapSide (floorIndex) {
    const VOID = NODES.VOID.code
    const HARDSTONE = NODES.HARDSTONE.code
    const HELLSTONE = NODES.HELLSTONE.code
    const SLATE = NODES.SLATE.code
    ...
    const supportRight = chunkManager.getTileAt(floorIndex + 1)
    const canRight = (supportRight === HARDSTONE || supportRight === HELLSTONE || supportRight === SLATE) &&
      chunkManager.isRectCode(cx, topY, 2, 3, VOID) &&
      blockedTiles.canPlaceRect(cx, topY, 2, 3)

    const supportLeft = chunkManager.getTileAt(floorIndex - 1)
    const canLeft = (supportLeft === HARDSTONE || supportLeft === HELLSTONE || supportLeft === SLATE) &&
      chunkManager.isRectCode(cx - 1, topY, 2, 3, VOID) &&
      blockedTiles.canPlaceRect(cx - 1, topY, 2, 3)
   ...
   * Cherche un nouvel emplacement pour le dernier record de #regrowQueue.
   * Départ VOID, descente jusqu'au premier non-VOID, sol HARDSTONE, HELLSTONE ou SLATE
   * (ancre, via #findInferncapFloor), puis test flat à droite (cx+1) et à gauche (cx-1)
   * via #findInferncapSide — 50/50 si les deux sont valides.
   * Si trouvé : finalise le record (present=true, byTile/#byChunk/#bySoil, blockPlacement,
   * persistence) et le retire de #regrowQueue (dernier élément, length--).
   * Replanifie tant que #regrowQueue n'est pas vide, avec INFERNCAP_REGROW_RETRY_DELAY_MS.
   */
  inferncapRegrow () {
    if (this.#regrowQueue.length === 0) return

    const floorIndex = this.#findInferncapFloor()
    const soilX = floorIndex !== 0 ? this.#findInferncapSide(floorIndex) : 0
    const found = soilX !== 0
    ...
    if (found) {
      ...
      this.#bySoil.set(soilIndex, record)
      this.#bySoil.set(soilIndex + 1, record)
      blockedTiles.blockPlacementRect(soilX, topY, 2, 3)
      saveManager.queueStaticUpdate({storeName: 'plant', record})
  ...
  /**
   * Liaison EventBus : 'world/tile-changed'.
   * Détruit l'inferncap présent si une des 6 tuiles VOID de son corps n'est plus libre,
   * ou si l'une des 2 tuiles sol n'est plus HARDSTONE, HELLSTONE ni SLATE.
   * @param {{tileIndex: number, tileOldCode: number, tileNewCode: number}} payload
   */
  onTileChangedInferncap ({tileIndex}) {
    // Cas 1 — tuile du corps : une des 6 VOID devient autre chose
    const byBodyRecord = this.byTile.get(tileIndex)
    if (byBodyRecord !== undefined) this.#destroyPresent(byBodyRecord)
...
export const inferncapSystem = new InferncapSystem()
