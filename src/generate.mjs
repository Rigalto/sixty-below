...
import {NODES, NODES_LOOKUP, NODE_TYPE, BIOME_TYPE, PLANT_KIND, PLANT_TYPE, ITEMS, TREE_IMAGES, PARSNIP_COUNT, MANDRAKE_COUNT, CACTUS_COUNT, BAMBOO_COUNT, OLEANDER_COUNT, SATANS_CUBE_COUNT, SNEAKTHORN_COUNT, CURSEDCROWN_COUNT, ABYSSHORN_COUNT, INFERNCAP_COUNT, SUNFLOWER_OAK_MIN_DIST} from '../assets/data/data.mjs'
 ...
 */
  placeAmbermirages (surfaceLine, guarded, initialWeather) {
    ...
    for (let x = 2; x < W - 2; x++) {
      ...
      if (worldBuffer.read(x - 1, surfaceLine[x - 1]) !== SAND) continue
      if (worldBuffer.read(x + 1, surfaceLine[x + 1]) !== SAND) continue
  ...
  placeParsnipsSunflowers (surfaceLine, guarded, oakPositions) {
    ...
    for (const x of spots) {
      ...
      for (const oakX of oakPositions) {
        if (Math.abs(x - oakX) <= SUNFLOWER_OAK_MIN_DIST) { nearOak = true; break }
      }
      if (nearOak) continue
