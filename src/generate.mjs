// generate.mjs — WorldBuffer - WorldGenerator - BiomesGenerator - BiomeNaturalizer - LiquidFiller, ClusterGenerator - TileGuard - WorldCarver - WebFiller - FurnitureGenerator - PlantGenerator
...
class WorldGenerator {
  async generate (seed, debug = false) {
    ...
    clusterGenerator.addNativeTopsoilClusters()
  ...
  async save (seed, {hives, cobwebCaves, geodeCaves, lakes, liquidBodies, fernsCaves, mossCaves, mushroomCaves, plants, pyramid, ruinedcabin, lostTemple, ancientHouse, leftBeach, rightBeach, antlions, anthills, termites, hearts, triskels, graveyard, weather, thornspineCount}) {
    ...
    await database.addMultipleRecords('plant', plants)
 ...
 */
  ...
  scatterClusters (x0, y0, x1, y1, percent, code, sizeMin = 5, sizeMax = 8) {
    ...
    const target = Math.max(1, Math.round(surface * percent))
    const result = []
    const seen = new Set() // index déjà posés — évite de compter un doublon comme une nouvelle tuile
    ...
    while (seen.size < target) {
      ...
      for (const tile of cluster) {
        if (seen.has(tile.index)) continue
        seen.add(tile.index)
        result.push(tile)
      }
    }
 ...
 * Le pourcentage cible directement la couverture réelle en tuiles (round(surface x percent)),
 * pas un nombre de clusters — les clusters sont ajoutés jusqu'à ce que ce nombre de tuiles
 * soit atteint, ou après TOPSOIL_SCATTER_ATTEMPTS_FACTOR x target tentatives (zone saturée,
 ...
 * avec des clusters de topsoil natif (DIRT/SAND/SILT), jusqu'à percent de couverture
 * réelle (NATIVE_TOPSOIL_MAP). Ne traite que les biomes présents dans la table —
 * les rectangles d'un biome absent de NATIVE_TOPSOIL_MAP sont ignorés.
 * Appelée avant addTopsoilClusters() : le natif forme la base, les intrusions
 * étrangères et HUMUS viennent ensuite se disperser par-dessus à percent faible.
 *
 * Prérequis : initZoneRects()
 */
  addNativeTopsoilClusters () {
    for (const rect of this.zoneRects) {
      const entry = NATIVE_TOPSOIL_MAP[rect.biome]
      if (!entry) continue
      this.applyTiles(this.scatterClusters(
 ...
 */
  ...
  /**
 * Creuse une Mushroom Cave rectangulaire bruitée par zone de biome FOREST.
 * Layer : caverns_top. Fond tapissé via #fillFernCaveFloor (GRASSMUSHROOM + HUMUS).
 ...
 */
  digMushroomCaves () {
    ...
    for (let i = 0; i < this.#zoneRects.length; i++) {
      ...
      const radiusX = seededRNG.randomGetMinMax(MUSHROOM_CAVE_RADIUS_X_MIN, MUSHROOM_CAVE_RADIUS_X_MAX)
      const radiusY = seededRNG.randomGetMinMax(MUSHROOM_CAVE_RADIUS_Y_MIN, MUSHROOM_CAVE_RADIUS_Y_MAX)

      let cx, cy, valid
      let attempts = 0
      do {
        cx = seededRNG.randomGetMinMax(rect.x0 + radiusX, rect.x1 - radiusX)
        cy = seededRNG.randomGetMinMax(rect.yUnder + radiusY, rect.yCavernsMid - radiusY)
        valid = !this.isExcluded(cx - radiusX, cy - radiusY, cx + radiusX, cy + radiusY)
        attempts++
      } while (!valid && attempts < MAX_ATTEMPTS)
      if (!valid) continue
  ...
  /**
 * Creuse une Pyramide en biome DESERT, layer under.
 * Structure en KHEPRITE + VOID définie par PYRAMID_WALL_INDEXES et PYRAMID_VOID_INDEXES.
 * Orientation aléatoire (gauche ou droite).
 * Prérequis : initZoneRects(), initExclusions().
 *
 * @returns {{room1: number, room2: number}|null} — index coin haut-gauche de chaque salle (encodage y<<10|x), ou null si MAX_ATTEMPTS épuisé
 */
  digPyramid () {
    const KHEPRITE = NODES.KHEPRITE.code
    const VOID = NODES.VOID.code
    const MAX_ATTEMPTS = 100
    const pyramids = []
    ...
    if (desertRects.length === 0) return pyramids
    ...
    return {
      room1: origin + (PYRAMID_ROOM1_DELTA.dy << 10) + r1dx,
      room2: origin + (PYRAMID_ROOM2_DELTA.dy << 10) + r2dx
    }
  }
 ...
 */
  digLostTemple () {
    ...
    const OLYMPITEWALL = NODES.OLYMPITEWALL.code
    const VOID = NODES.VOID.code
    const MAX_ATTEMPTS = 100
    const TEMPLE_W = 15
    const TEMPLE_H = 10
    ...
    const columnTiles = []
    for (const delta of TEMPLE_RUIN_COLUMNS_INDEXES) {
      const dx = delta & 0x3FF
      const dy = delta >> 10
      const x = tx + dx
      const y = ty + dy
      columnTiles.push({x, y, index: (y << 10) | x, code: OLYMPITEWALL})
    }
 ...
 */
  digAncientHouse () {
    ...
    const ROOF_ITEMS = ['tableWood', 'blastFurnace', 'sawmill', 'forge', 'stoneBench', 'loom', 'tanningRack', 'jewelerBench', 'alchemyTable', 'cookingPot']
    ...
    const GROUND_ITEMS = ['tableWood', 'blastFurnace', 'sawmill', 'forge', 'stoneBench', 'loom', 'tanningRack', 'jewelerBench', 'alchemyTable', 'cookingPot']
...
class FurnitureGenerator {
   ...
   */
  #createEmptySlots (container, capacity, furnitureId = '') {
    for (let i = 0; i < capacity; i++) {
      this.#inventory.push({container, item: '', count: 0, prefix: '', furnitureId, slot: i, locked: false, deleted: false})
    }
  }
   ...
   * @param {number} slot
   */
  #addInHotbar (item, count, prefix, slot) {
 ...
 */
  ...
  addFurnitureAt (index, code) {
    ...
    const furniture = {id, index, code, stype, w, h, blocked: 0, deleted: false}
 ...
 */
  addInChest (chest, item, count, prefix = '') {
    if (count === 0) return
    const record = this.#inventory.find(s => s.furnitureId === chest.id && s.item === '')
    if (record === undefined) return
    record.item = item
    record.count = count
    record.prefix = prefix
...
export const furnitureGenerator = new FurnitureGenerator()
 ...
 */
  #placeOneCoconut (soilIndex) {
    ...
    const hasNutInTree = seededRNG.randomGetBool()
    const cycleDelay = (COCONUT_CYCLE_DELAY * seededRNG.randomGetRealMinMax(0.01, 1.2)) | 0
    ...
    this.#plants.push({
      ...
      hasNutInTree,
      treeTimestamp: 480000 + cycleDelay,
      treeNextAction: hasNutInTree ? 'fall' : 'grow',
  ...
  placeThornspines (surfaceLine, guarded, biomesDescription) {
    ...
    while (placed < quota && spots.length > 0) {
      ...
      const x = spots[idx]
      removeIndexUnordered(spots, idx)
 ...
 */
  placeTrees (surfaceLine, guarded) {
    ...
    const placeTree = (soilX, y, grassCode) => {
      ...
      if (size < 4) {
        growthTimestamp = 480000 + seededRNG.randomGetMinMax(1000, (1.2 * growthBase) | 0)
      }
 ...
 */
  ...
  placeOleanders (zoneRects) {
    ...
    while (placed < OLEANDER_COUNT && consecutiveFailures < MAX_ATTEMPTS) {
      const rect = seededRNG.randomGetArrayValue(rects)
  ...
  placeSatansCubes (zoneRects) {
    ...
    const rects = []
    for (const rect of zoneRects) {
      if (rect.biome !== BIOME_TYPE.FOREST && rect.biome !== BIOME_TYPE.DESERT) continue
    ...
    while (placed < SATANS_CUBE_COUNT && consecutiveFailures < MAX_ATTEMPTS) {
      const rect = seededRNG.randomGetArrayValue(rects)
      const cx = seededRNG.randomGetMinMax(rect.x0 + 2, rect.x1 - 2)
      const cy = seededRNG.randomGetMinMax(rect.y0 + 1, rect.y1 - 1)
  ...
  placeCursedcrowns (zoneRects) {
    ...
    const rects = []
    for (const rect of zoneRects) {
      if (rect.biome !== BIOME_TYPE.JUNGLE && rect.biome !== BIOME_TYPE.DESERT) continue
      rects.push({x0: rect.x0, x1: rect.x1, y0: rect.yUnder, y1: rect.yCaverns})
    }
    if (rects.length === 0) return
    ...
    while (placed < CURSEDCROWN_COUNT && consecutiveFailures < MAX_ATTEMPTS) {
      const rect = seededRNG.randomGetArrayValue(rects)
      const cx = seededRNG.randomGetMinMax(rect.x0 + 2, rect.x1 - 2)
      const cy = seededRNG.randomGetMinMax(rect.y0 + 1, rect.y1 - 1)
  ...
  placeAbysshorns (zoneRects) {
    ...
    const rects = []
    for (const rect of zoneRects) {
      rects.push({x0: rect.x0, x1: rect.x1, y0: rect.yUnder, y1: rect.yCavernsMid})
    }
    if (rects.length === 0) return
  ...
  placeInferncaps (zoneRects) {
    ...
    const rects = []
    for (const rect of zoneRects) {
      rects.push({x0: rect.x0, x1: rect.x1, y0: rect.yCavernsMid, y1: rect.yCaverns})
    }
    if (rects.length === 0) return
