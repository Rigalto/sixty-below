...
| `placedGuard` | `PlacedGuard` | Registre des tuiles occupées par les objets placés (meubles, puis plantes). `init()`, `has(index)`, `add(index)`, `addRect(x1,y1,x2,y2)`, `hasRect(x1,y1,x2,y2)`. Consulté par `placeSeaChests`, `placeCavernChests`, `placeUndergroundChests`, `placeSurfaceChests` et tous les placements de plantes. |
