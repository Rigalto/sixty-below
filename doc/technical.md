...
| `applyTiles` | `(tiles: Array<{x, y, index, code}>): {x1, y1, x2, y2}` | Écrit les tuiles dans `worldBuffer` via `writeAt`. Protège : ETERNAL (FOG, DEEPSEA, BASALT, LAVA), SKY, tous les liquides (WATER, SEA, HONEY, SAP), et les nodes CREATION (LAKE_*_SIDE, LAKE_*_BED...). Retourne le rectangle englobant  des tuiles réellement écrites.. |
| `applyTilesOverSky(tiles)` | Variante de `applyTiles` autorisant l'écrasement de SKY. Protège uniquement ETERNAL (FOG, DEEPSEA, BASALT, LAVA). Utilisée pour le creusement des lacs de surface. Retourne le rectangle englobant `{x1, y1, x2, y2}`. |
...
| `digLakes()` | Creuse un lac de surface par zone de biome (Oasis en Desert). Double ellipse : corps principal + fosse centrale bruitée avec offset aléatoire. Bords/fond protégés par nodes CREATION spécifiques au biome. Remplissage WATER différé. Prérequis : `initZoneRects()`. Retourne `Array<{cx, cy, biome}>`. |
