...
* `kind` HERB : liste des spots d'herbes (Parsnip, Sunflower) ou des herbes présentes dans le monde (Mandrake, Pricklepad, Coral, Parsnip, Sunflower, Fireblossom, Oleander, Skorn, Ambermirage, Gravelweed, Bamboo...)
...
| `addNativeTopsoilClusters()` | Recouvre la bande de surface de chaque zone biome avec des clusters de topsoil natif (NATIVE_TOPSOIL_MAP), jusqu'à percent de couverture réelle. Prérequis : initZoneRects(). À appeler avant addSubstratClusters(). |
...
| `placedGuard` | `PlacedGuard` | Registre des tuiles occupées par les objets placés (meubles, puis plantes). `init()`, `has(index)`, `add(index)`, `addRect(x1,y1,x2,y2)`. Consulté par `placeSeaChests`, `placeCavernChests`, `placeUndergroundChests`, `placeSurfaceChests` et tous les placements de plantes. |
...
| `plantGenerator`       | `PlantGenerator`      | Ajout de la flore dans le monde. `init()`, `get plants()`, `placeSeaCoconut(beachRect, surfaceLine, isLeft, guarded)`, `placeOasisCoconut(lake, surfaceLine, guarded)`, `placeCorals(seaRect)`, `placeTrees(surfaceLine, guarded)`, `placeGiantMushrooms(mushroomPlants)`, `spreadNatural(surfaceLine, naturalCode, topsoilCode, plantType)`, `spreadMushroom(naturalPlants, naturalCode, topsoilCode, gazCode, plantType)`, `placeAmbermirages(surfaceLine, guarded, initialWeather)`, `.placeParsnipsSunflowers(surfaceLine, guarded, oakPositions)`, `placeBloodmoons(surfaceLine, guarded)`, `placeFerns(fernsPlants):count`, `placeMoss()`, `placeCaveMushrooms(mushroomPlants, initialWeather, giantOccupied)`, `placeMandrakes(zoneRects)`, `placePricklepads(zoneRects)`, `placeBamboo(zoneRects)`, `placeGravelweed(zoneRects)`, `placeOleanders(zoneRects)`, `placeSatansCubes(zoneRects)`, `placeSneakthorns(zoneRects)`, `placeCursedcrowns(zoneRects)`, `placeAbysshorns(zoneRects)`, `placeInferncaps(zoneRects)`, `placeThornspines(surfaceLine, guarded, biomesDescription):count` |
...
| `getCenterTile` | `(): {x, y, direction}` | Retourne la tuile du centre de la hitbox en coordonnées tuile. |
