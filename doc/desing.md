...
- CSS `classList.add('fading')` gère le fondu en 300 ms
- Buff `playerFreeze` positionné → buff composé `player-movement` retourne 0 → déplacement bloqué
- Buff `playerFreeze` positionné → gel de toute action future et tentative d'arrêt des actions en court
- Lancement de la  phase 2 avec 300ms de délai (`taskScheduler`)

**Phase 2** — `onTeleportPhase2()` :
- Positionnement du joueur à sa nouvelle position → la caméra recalcule, les images des chunks se régénèrent pendant la seconde d'attente
- Lancement de la  phase 3 avec 1000ms de délai (`taskScheduler`)

**Phase 3** — `onTeleportPhase3()` :
- CSS `classList.remove('fading')` gère le fondu inverse en 300 ms
- Buff `playerFreeze` supprimé → déplacement restauré et actions autorisées
